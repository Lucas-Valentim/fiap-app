...
import fiap.com.br.fiapapp.business.Functions
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cmbMarca.adapter = adapter  */
               ...
               docMarca.get().addOnCompleteListener(OnCompleteListener {
                    ...
                    val adapterMarca =  ArrayAdapter(this, android.R.layout.simple_spinner_item, spinnerArrayMarca)
        ...
        docCor.get().addOnCompleteListener(OnCompleteListener {
            ...
            val adapterCor =  ArrayAdapter(this, android.R.layout.simple_spinner_item, spinnerArrayCor)
        ...
        docEmpresa.get().addOnCompleteListener(OnCompleteListener {
            ...
            for (dataObject in it.getResult()!!.documents){
                ...
                spinnerArrayEmpresa.add(empresa.cod_empresa.toString() + ";" + empresa.cidade +
                                    ";" + empresa.estado + ";" + empresa.razao_social)

            }

            val adapterEmpresa =  ArrayAdapter(this, android.R.layout.simple_spinner_item, spinnerArrayEmpresa)
        ...
        fun carregarModelo(idMarcaSel: Number): SpinnerAdapter {
            ...
            val adapterModelo = ArrayAdapter(this,android.R.layout.simple_spinner_dropdown_item, spinnerArrayModelo )
        ...
        cmbMarca.onItemSelectedListener = object:
            ...
            btnFiltrar.setOnClickListener {
                ...
                if (msg.isNotEmpty()) {
                    ...
                    if (cmbMarca.selectedItem != null){ nomeMarca = cmbMarca.selectedItem.toString()}
                    if (cmbModelo.selectedItem != null){ nomeModelo = cmbModelo.selectedItem.toString()}
                    if (cmbCor.selectedItem != null){ cor = cmbCor.selectedItem.toString()}
                    if (cmbFilial.selectedItem != null){ filial = cmbFilial.selectedItem.toString()}

                    var checkAno = dtaAno.text
                    if (!checkAno.isNullOrEmpty()){ anoModelo = dtaAno.text.toString().toInt()}

                    var checkKm = etnKm.text
                    if (!checkKm.isNullOrEmpty()){ km = etnKm.text.toString().toLong()}

                    var intent = Intent(this, Lista::class.java)
                       intent.putExtra("marca", nomeMarca)
                       intent.putExtra("modelo", nomeModelo)
                       intent.putExtra("cor", cor)
                       intent.putExtra("filial", filial)
                       intent.putExtra("ano", anoModelo)
